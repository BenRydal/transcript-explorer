				...
				if (((t.dynamicTyping = r), (t.transform = !!q(t.transform) && t.transform), t.worker && b.WORKERS_SUPPORTED)) {
					var i = (function () {
						...
						var e =
								...
								(i = s.toString()),
								b.BLOB_URL || (b.BLOB_URL = r.createObjectURL(new Blob(['(', i, ')();'], { type: 'text/javascript' })))),
							t = new f.Worker(e);
						var r, i;
		...
		d.fn.parse = function (o) {
			...
			return (
				this.each(function (e) {
					if (
						!('INPUT' === d(this).prop('tagName').toUpperCase() && 'file' === d(this).attr('type').toLowerCase() && f.FileReader) ||
						!this.files ||
						0 === this.files.length
					)
						return !0;
	...
	function c(e) {
		...
		(this.stream = function (e) {
			(this._input = e),
				...
				s ? (((i = new FileReader()).onload = y(this._chunkLoaded, this)), (i.onerror = y(this._chunkError, this))) : (i = new FileReaderSync()),
				this._nextChunk();
		}),
	...
	function r(g) {
		...
		function m() {
			if (
				(c &&
					h &&
					(k('Delimiter', 'UndetectableDelimiter', "Unable to auto-detect delimiting character; defaulted to '" + b.DefaultDelimiter + "'"),
					(h = !1)),
				g.skipEmptyLines)
			)
					...
					for (g = M, M++; ; ) {
						...
						if (O !== z || a[g + 1] !== z) {
							if (O === z || 0 === g || a[g - 1] !== z) {
								...
								u.push({ type: 'Quotes', code: 'InvalidQuotes', message: 'Trailing quote on quoted field is malformed', row: h.length, index: M }),
									g++;
							}
						} else g++;
					}
	...
	function _(e) {
		...
		if (t.error) r.userError(t.error, t.file);
			...
			if (q(r.userStep)) {
				for (
					var s = 0;
					s < t.results.data.length && (r.userStep({ data: t.results.data[s], errors: t.results.errors, meta: t.results.meta }, n), !i);
					s++
				);
				delete t.results;
