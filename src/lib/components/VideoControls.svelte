	...
	<div class="video-controls" on:mousedown|stopPropagation on:click|stopPropagation role="toolbar" tabindex="0" on:keydown|stopPropagation>
		...
		>
			<div class="progress-track">
				<div
					class="progress-fill"
					style="width: {isScrubbing ? (scrubTime / duration) * 100 : progressPercent}%"
				></div>
				<div
					class="progress-handle"
					style="left: {isScrubbing ? (scrubTime / duration) * 100 : progressPercent}%"
				></div>
			</div>
		</div>
		...
		<!-- Fullscreen toggle -->
		<button class="control-btn" on:click={handleFullscreenToggle} aria-label={isFullscreen ? 'Exit fullscreen' : 'Fullscreen'} title={isFullscreen ? 'Exit fullscreen' : 'Fullscreen'}>
			{#if isFullscreen}
				<svg viewBox="0 0 24 24" fill="currentColor">
